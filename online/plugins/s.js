(function () {
    ...
    function menu$2(target, card_data) {
      ...
      Lampa.Select.show({
        ...
        onSelect: function onSelect(m) {
          if (m.model) {
            Lampa.Activity.push({
              url: Lampa.Platform.is('android') && window.blazor_init != undefined ? card_data.model.uri : (Defined.localhost.replace('/sisi', '') + '/' + card_data.model.uri),
    ...
    function ApiPWA() {
	  ...
	  this.bookmark = function (element, add, call) {
        var u = '/bookmark/' + (add ? 'add' : 'remove?uid=' + element.bookmark.uid);
        network.silent(Defined.localhost + this.account(u, true), function (e) {
          call(true);
        }, function () {
          call(false);
    ...
    function ApiHttp() {
      ...
      this.account = function (u) {
        if (u.indexOf('/bookmark') == -1) return u;
        var unic_id = Lampa.Storage.get('sisi_unic_id', '');
        var email = Lampa.Storage.get('account', {}).email;
        if (u.indexOf('box_mac=') == -1) u = Lampa.Utils.addUrlComponent(u, 'box_mac=' + unic_id);else u = u.replace(/box_mac=[^&]+/, 'box_mac=' + unic_id);
    ...
    var Api = ApiPWA$1;
	//var Api = window.blazor_init == undefined ? ApiHttp$1 : ApiPWA$1;
	
	/*
	var counter = 0;
	var intervalId = setInterval(function() {
		if (counter >= 200) { // 40 секунд
			clearInterval(intervalId);
		} else {
			if (window.blazor_init !== undefined) {
				Api = ApiPWA$1;
				clearInterval(intervalId);
			}
			counter++;
